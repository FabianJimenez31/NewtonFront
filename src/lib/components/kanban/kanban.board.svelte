...
<script lang="ts">
	...
	const pipelineMetrics = $derived(
		calculatePipelineMetrics($kanbanStore.boardData),
	);
	...
	onMount(async () => {
		...
		if (!$authStore.token) {
			console.error(
				"[KANBAN BOARD] No token available, cannot load board",
			);
		...
		try {
			...
			console.log("[KANBAN BOARD] Config loaded:", {
				...
				stages: $kanbanStore.stages.map((s) => ({
					id: s.id,
					name: s.name,
			...
			if (autoRefresh) {
				console.log(
					"[KANBAN BOARD] Starting auto-refresh (30s interval)",
				);
	...
	async function loadBoard() {
		try {
			...
			error = null;
			console.log(
				"[KANBAN BOARD] Starting loadBoard with filters:",
				filters,
			);
			...
			console.log("[KANBAN BOARD] Board data loaded:", {
				...
				stageCount: $kanbanStore.boardData
					? Object.keys($kanbanStore.boardData.stages).length
					: 0,
				stages: $kanbanStore.boardData
					? Object.keys($kanbanStore.boardData.stages)
					: [],
				visibleStages: $visibleStages.map((s) => ({
					id: s.id,
					name: s.name,
				})),
			});
	...
	async function handleDrop(lead: LeadKanban, targetStageId: string) {
		try {
			...
			if (!validation.is_valid) {
				console.error(
					"[KANBAN BOARD] Invalid transition:",
					validation.message,
				);
			...
			await moveLeadToStage($authStore.token!, lead.id, {
				stage: targetStageId,
			});
...
<div class="flex flex-col h-full px-3 sm:px-6 pb-4 {className}">
	...
	{#if error}
		<div
			class="bg-destructive/10 text-destructive px-4 py-3 rounded-lg mb-4"
		>
			<div class="flex items-start gap-2">
				...
				<div class="flex-1">
					...
					<div class="mt-2 flex gap-2 flex-wrap">
						...
						>
							<RefreshCw
								class="h-3 w-3 mr-1 {isRefreshing
									? 'animate-spin'
									: ''}"
							/>
							Reintentar
	...
	{:else}
		...
		<div class="relative flex-1 overflow-x-auto">
			{#if showBoardLoader}
				<div
					class="absolute inset-0 z-20 flex flex-col items-center justify-center gap-3 bg-background/70 backdrop-blur-sm"
				>
					<div
			...
			{/if}
			<div
				class="flex gap-4 h-full pb-4 min-w-min pr-2 sm:pr-4 pl-1 sm:pl-2"
			>
				{#each $visibleStages as stage (stage.id)}
					{@const leads =
						$kanbanStore.boardData.stages[stage.id] || []}
					<KanbanColumn
						{stage}
						{leads}
						{onLeadClick}
